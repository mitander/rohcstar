...
use rohcstar::crc::CrcCalculators; // Import CrcCalculators
...
fn p1_ir_packet_with_wrong_profile_id_fails() {
    ...
    if ir_packet_bytes.len() > 1 {
        // For CID 0, profile ID is at index 1 (after Type octet 0xFD)
...
fn p1_ir_packet_too_short_fails() {
    ...
    let min_valid_ir_len = 1 + 1 + P1_STATIC_CHAIN_LENGTH_BYTES + 1; // Type+Profile+Static+CRC for IR-STATIC

    for len in 0..ir_packet_bytes_full.len() {
        if len < min_valid_ir_len {
            // Only test lengths certainly too short for any IR
...
fn p1_compressor_context_static_fields_remain_constant() {
    ...
    for i in 1..=3 {
        ...
        headers_next.ip_identification = headers1.ip_identification;
...
fn p1_multiple_flows_different_cids() {
    ...
    assert_eq!(compressed2_flow1[1] & 0x80, 0x00);
    ...
    assert_eq!(decompressed2_flow1.ip_identification, 0);
}
...
fn p1_ssrc_change_forces_context_reinitialization_and_ir() {
    ...
    assert_eq!(compressed3[0], P1_ROHC_IR_PACKET_TYPE_WITH_DYN);
...
fn p1_ir_refresh_interval_edge_cases() {
    ...
    for i in 2..=10 {
        ...
        let compressed = handler
            ...
            .unwrap();
        assert_ne!(compressed[0], P1_ROHC_IR_PACKET_TYPE_WITH_DYN);
    ...
    let compressed_ir1_1 = handler
        ...
        .unwrap();
    assert_eq!(compressed_ir1_1[0], P1_ROHC_IR_PACKET_TYPE_WITH_DYN);
    ...
    let compressed_ir1_2 = handler
        ...
        .unwrap();
    assert_eq!(compressed_ir1_2[0], P1_ROHC_IR_PACKET_TYPE_WITH_DYN);
    ...
    let compressed_refresh_ir = handler
        ...
        .unwrap();
    assert_eq!(compressed_refresh_ir[0], P1_ROHC_IR_PACKET_TYPE_WITH_DYN);
...
fn p1_ir_packet_with_static_only_d_bit_0_parse_successfully() {
    ...
    let ir_data = create_ir_packet_data(0, 0xFEEDF00D, 10, 100);
    ...
    let crc_payload_slice = &ir_packet_bytes[1..];
    ...
    match result {
        Ok(GenericUncompressedHeaders::RtpUdpIpv4(h)) => {
            ...
            assert_eq!(h.rtp_sequence_number, 0); // Dynamic fields are zeroed for IR-Static
            assert_eq!(h.rtp_timestamp, Timestamp::new(0));
        }
        Ok(other) => panic!(
            // Catch other Ok variants like TestRaw
...
fn p1_decompressor_context_persistence_across_ir_packets() {
    ...
    assert_eq!(decomp_ctx2.mode, Profile1DecompressorMode::FullContext);
    ...
    assert_eq!(decomp_ctx2.consecutive_crc_failures_in_fc, 0);
}
