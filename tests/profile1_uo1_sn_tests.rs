...
fn p1_uo1_sn_max_sn_jump_encodable() {
    ...
    let headers_jump_pos =
        create_rtp_headers(sn_jump_pos, initial_ts_val + 100, initial_marker, ssrc)
    ...
    let compressed_jump_pos = engine
        ...
        .unwrap();
    assert_eq!(compressed_jump_pos.len(), 3);
    ...
    let decomp_jump_pos_generic = decomp_jump_pos_result.unwrap(); // Store the owned value
    let decomp_jump_pos = decomp_jump_pos_generic
        .as_rtp_udp_ipv4() // Now borrow from the owned value
        .unwrap()
        .clone(); // Clone if you need an owned RtpUdpIpv4Headers, otherwise use the reference
    ...
    let compressed_jump_neg = engine
        ...
        .unwrap();
    assert_eq!(compressed_jump_neg.len(), 3);
    ...
    match decompress_result_neg_jump {
        ...
        Ok(h) => panic!("Expected CrcMismatch, but got Ok({:?})", h),
...
fn p1_uo1_sn_prefered_over_uo0_for_larger_sn_diff() {
    ...
    let sn_force_uo1 = sn_uo0_max + 16;
    let headers_uo1 = create_rtp_headers(sn_force_uo1, initial_ts_val + 20, initial_marker, ssrc);
    ...
    assert_eq!(decomp_uo1.rtp_timestamp, Timestamp::new(initial_ts_val));
}
