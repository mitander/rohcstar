...
mod common;
use common::{
    assert_decompressor_mode, create_rtp_headers_fixed_ssrc, establish_ir_context,
...
fn p1_umode_ir_to_fo_sequence_cid0() {
    ...
    let original_headers2 = create_rtp_headers_fixed_ssrc(101, 1160, false);
    ...
    assert!(!decomp_headers2.rtp_marker, "P2_UO0: Context marker");
    assert_eq!(
    ...
    let original_headers3 = create_rtp_headers_fixed_ssrc(102, 1320, true);
    ...
    assert_eq!(
        ...
        1000 // From context, not from packet 3
    );

    // --- Packet 4: UO-1-SN (SN=103, M=false) ---
    let original_headers4 = create_rtp_headers_fixed_ssrc(103, 1480, false);
    ...
    let original_headers5 = create_rtp_headers_fixed_ssrc(104, 1640, false);
    ...
    assert_eq!(
        comp_ctx.fo_packets_sent_since_ir, 0,
...
fn p1_umode_ir_to_fo_sequence_small_cid() {
    ...
    let decomp_ctx = get_decompressor_context(&engine, small_cid);
    assert_eq!(decomp_ctx.cid, small_cid);
    ...
    let original_headers2 = create_rtp_headers_fixed_ssrc(201, 2160, false); // Marker changed
    let generic_headers2 = GenericUncompressedHeaders::RtpUdpIpv4(original_headers2.clone());
    ...
    let original_headers3 = create_rtp_headers_fixed_ssrc(202, 2320, true); // Marker changed
    let generic_headers3 = GenericUncompressedHeaders::RtpUdpIpv4(original_headers3.clone());
...
fn p1_umode_sn_jump_triggers_uo1() {
    ...
    let headers2 = create_rtp_headers_fixed_ssrc(501, 5160, false);
...
fn p1_umode_uo0_sn_decoding_with_simulated_packet_loss() {
    ...
    assert_eq!(decomp_ctx.mode, Profile1DecompressorMode::FullContext);
    assert_eq!(decomp_ctx.p_sn, DEFAULT_WLSB_P_OFFSET);

    // Simulate loss of SN=101 and SN=102 by compressing but not decompressing
    let headers_sn101 = create_rtp_headers_fixed_ssrc(101, 1160, false);
    let _rohc_lost_packet1 = engine
        ...
        .unwrap();
    let headers_sn102 = create_rtp_headers_fixed_ssrc(102, 1320, false);
    ...
    let headers_sn103 = create_rtp_headers_fixed_ssrc(103, 1480, false);
    ...
    let headers_sn104 = create_rtp_headers_fixed_ssrc(104, 1640, false);
